...
void Report::initialBest(const Metrics& best) const {
    std::cout << "Initial best: time=" << best.time_us
              << " µs, mem=" << best.memory_kb
              << " KB, fitness=" << best.fitness() << "\n";
}

void Report::candidate(int gen, const Metrics& candidate, const Metrics& best) const {
...
void Report::newBest(int gen, const Metrics& best) const {
    std::cout << "\nNew best! Gen " << gen << ": "
