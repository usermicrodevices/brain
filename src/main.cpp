...
int main() {
    ...
    // Read the initial core source
    std::string ownSource = Core::readOwnSource();

    Compiler compiler;
    Serializer serializer(ownSource, 0, Compiler::Metrics{0,0,false});
    Reporter reporter;
    Benchmark benchmark(compiler);

    std::string bestSource = ownSource;
    Compiler::Metrics best = benchmark.run(bestSource);
    reporter.initialBest(best);
    serializer.updateBest(bestSource, best, 0);
    ...
    while (!exit_requested) {
        ...
        std::string mutated = Core::mutate(bestSource);
        if (exit_requested) break;

        g_child_pid = 0;
        Compiler::Metrics m = benchmark.run(mutated);
        if (exit_requested) break;

        if (m.valid && m.fitness() < best.fitness()) {
            best = m;
            bestSource = mutated;
            serializer.updateBest(bestSource, best, generation);
